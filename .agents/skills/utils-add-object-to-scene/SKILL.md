...
Stateful object constructors receive an identity or readonly state slice. Their methods emit intent or synchronize presentation; they never own or directly replace authoritative state. Generate architecture artifacts only when the requested task explicitly includes them, using `arch-make-data-logical-diag` when a logical authoritative-state diagram is requested.
