...
- [x] Utworzyć projekt Cloudflare Pages **Direct Upload** `milky-way-trader`, z produkcyjnym branchem `main`. Korzystać z przydzielonej domeny `pages.dev`. Direct Upload jest zgodny z wybranym CI; późniejsza zmiana na Git integration wymaga nowego projektu. [Dokumentacja Cloudflare](https://developers.cloudflare.com/pages/get-started/direct-upload/).
...
- [x] Utworzyć token Cloudflare z uprawnieniem Pages Edit ograniczonym do wybranego konta.
- [x] Wdrożyć lokalnie zbudowane `dist/` jako preview brancha `staging` i sprawdzić demo. Pierwszy push uruchomił nieudaną próbę produkcji; po utworzeniu projektu i preview ponowienie CI opublikowało produkcję. Testy obu środowisk zakończone sukcesem.
...
- [x] Token Cloudflare zapisać w GitHub Actions Secrets, account ID w Variables. Token przekazywać tylko do kroku publikacji lokalnym Wranglerem.
- [x] Przygotować selektywny commit zmian wdrożeniowych. Zachować obecną historię Git i istniejące zmiany użytkownika.
- [x] `git push origin main` wykonać po jawnej zgodzie wymaganej przez `AGENTS.md`. Zgoda użytkownika „okay”; commit `ced3d25` przesłany.
- [x] Zapisać adresy Cloudflare, wersje narzędzi, wynik weryfikacji oraz identyfikator pierwszego udanego deploymentu. Identyfikatory Supabase/Google pozostają do uzupełnienia po potwierdzeniu konfiguracji.
